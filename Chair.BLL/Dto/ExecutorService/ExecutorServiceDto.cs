...
namespace Chair.BLL.Dto.ExecutorService
{
    public class ExecutorServiceDto : BaseDto
    {
        ...
        public decimal Price => Orders.Any(x => x.StarDate >= DateTime.UtcNow) ? decimal.Round(Orders.Where(x => x.StarDate >= DateTime.UtcNow).Average(x => x.Price ?? 0), 2) : 0;
        ...
        public DateTime Duration => Orders.Any(x => x.StarDate >= DateTime.UtcNow) ? DateTime.MinValue + TimeSpan.FromMinutes(Orders.Where(x => x.StarDate >= DateTime.UtcNow).Select(service =>
